...
// ---- Isolated timer sub-components (prevent parent re-renders) ----

/** Mission clock — ticks every 1s, isolated */
const MissionClock = memo(function MissionClock({ hudColor }: { hudColor: string }) {
...
/** Decorative drifting values — ticks every 3s, isolated */
const DriftingReadouts = memo(function DriftingReadouts({ hudColor }: { hudColor: string }) {
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center gap-0.5">
        ...
        <div
          ...
          style={{
            ...
            boxShadow: 'inset 0 1px 3px rgba(0, 0, 0, 0.5), 0 0 0 1px rgba(140, 170, 210, 0.05)',
          }}
        >
          ...
          <div
            ...
            style={{
              width: '1px',
              height: '10px',
              background: `linear-gradient(0deg, transparent, ${hudColor})`,
              transformOrigin: '50% 100%',
              transform: `translate(-50%, -10px) rotate(${heading}deg)`,
              opacity: 0.6,
          ...
          <div
            ...
            style={{ background: 'radial-gradient(circle at 35% 35%, rgba(180, 200, 230, 0.15), rgba(40, 50, 70, 0.4))' }}
          />
          <div
            className="absolute inset-[2px] rounded-full"
            style={{ background: 'linear-gradient(135deg, rgba(200, 220, 255, 0.04) 0%, transparent 50%)' }}
          />
        </div>
        <div className="font-mono" style={{ fontSize: '6px', color: hudColor, opacity: 0.45 }}>
          {heading.toFixed(1)}°
        </div>
      </div>
          ...
          <div
            ...
            style={{
              ...
              background: fuelPercent > 25
                ? `linear-gradient(0deg, ${hudColor}, rgba(0, 200, 220, 0.3))`
                : 'linear-gradient(0deg, #ef4444, rgba(239, 68, 68, 0.3))',
              opacity: 0.5,
              boxShadow: `0 0 3px ${fuelPercent > 25 ? hudColor : '#ef4444'}`,
            }}
          />
          <div
            className="absolute inset-0 rounded-sm"
            style={{ background: 'linear-gradient(90deg, rgba(200, 220, 255, 0.03) 0%, transparent 40%)' }}
          />
        </div>
        <div className="font-mono" style={{ fontSize: '6px', color: hudColor, opacity: 0.45 }}>
      ...
      <div className="flex flex-col items-center gap-0.5">
        ...
        <div className="relative w-[28px] h-[16px] overflow-hidden">
          <svg viewBox="0 0 28 16" className="w-full h-full">
            <path d="M 3 15 A 12 12 0 0 1 25 15" fill="none" stroke="rgba(80, 120, 160, 0.1)" strokeWidth="2" />
            <path
            ...
            <line
              ...
              stroke={hudColor} strokeWidth="0.8" opacity={0.5}
            />
 ...
 * Timer-driven updates isolated into memo'd sub-components to prevent
 * re-rendering the entire strip (and its sibling components) on every tick.
 */
  ...
  return (
    <div className="absolute top-6 left-0 w-[50px] bottom-[220px] z-15 pointer-events-none">
      ...
      <div className="relative flex flex-col items-center pt-8 gap-5 h-full px-1">
        <MissionClock hudColor={colors.hud} />
        <DriftingReadouts hudColor={colors.hud} />
      </div>
    </div>
  );
}
